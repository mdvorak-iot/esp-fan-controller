...
static void setup_init()
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(nvs_open(APP_CONFIG_NVS_NAME, NVS_READONLY, &handle));
    serialization_context_nvs nvs_context(handle);
    err = app_config_load(&nvs_context, &app_config);
    if (err != ESP_OK)
    {
