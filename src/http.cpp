...
static std::vector<uint16_t> rpms_;
static std::vector<apptemps::temperature_sensor> temps_;
...
esp_err_t getMetricsHandler(httpd_req_t *req)
{
    ...
    apptemps::temperature_values(temps_);
    for (auto t : temps_)
    {
    ...
    rpmcounter::rpm_counter_values(rpms_);
    for (size_t i = 0; i < rpms_.size(); i++)
    {
        m << "esp_rpm{hardware=\"" << hardware_ << "\",sensor=\"Fan " << i << "\"} " << rpms_[i] << '\n';
    }
